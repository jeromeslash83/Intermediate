...
def create_spend_chart(categories):
    ...
    percentage_list =[round(((spent/total)*100)//1) for spent in spending]
    ...
    for x in the_categories:
        if len(x) > max_length:
            max_length = len(x) 
    categs = [category for category in the_categories]
    ...
    return (heading + percent_chart + footer + string).rstrip('\n')